...
class LocationTrackingService : Service() {
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    // ✅ CRITICAL FIX: Always call startForeground() first to prevent crash
    // Android requires foreground service to call startForeground() within 5 seconds
    val notification =
    ...
    if (intent?.getStringExtra("ACTION") == "UPDATE_INTERVAL") {
      ...
      return START_STICKY // Keep service running after config update
    }
